...
## Round 35 — "driver" was already defined; §18.3 ratified and transcribed (2026-09-06)

| # | Topic | Ruling / outcome |
|---|---|---|
| R35-a | **Owner asked whether "driver" is defined anywhere in SPEC** | It is. `§ 4.2`, first line under the loop: *"A driver is the unit of work inside the orchestrator that advances one owned run."* `Appendix A` indexes it as *"Driver / the driving loop — §4.2"*. It is not in `§ 3` because `§ 3.1` enumerates **actors** — operator, orchestrator, planner, worker — and a driver is not an actor: it is a unit of work inside one of them, which is why it is defined where the loop it drives is defined. **No SPEC change made or needed**, and none was proposed |
| R35-b | **Owner ruled: §18.3 is accepted — write it into SPEC** | Transcribed. `CLAUDE.md § 2` rule 1 permits it: the owner directed the edit, and the text follows what round 34 built and reported, leg for leg |
| R35-c | **The same consequential sentence updated a second time** | `§ 18`'s closing paragraph now reads *"α's, γ's and β's are written; the rest are written when that milestone starts."* Same reason as R33-b — adding a demo script makes the previous count false, and a self-contradicting authority document is worse than either reading |
| R35-d | **What § 18.3 fixes that the code did not already** | Three things are now rules rather than choices this agent made: `restart: "no"` on the orchestrator, because a runtime that restarts it takes the demonstration away from the operator; β's faster coordination timings, with the reason (`§ 8.5` cannot claim from a live owner, so every kill waits for the lease) and the limit (they change how *long* failover takes, never what recovery *does*); and the two situations of `§ 13.1` that β does not demonstrate, `13.1.2` and `13.1.6`, each with its reason. That last is the point of writing it down at all — an unlisted absence reads as an oversight |
| R35-e | **Still outstanding** | `CLAUDE.md § 4` **step 5** for β — the owner's own hand-run of `demos/beta/demo.sh`. Nothing else |

---

## Round 36 — β hand-verified; δ is next, and is the first milestone with real implementation work (2026-09-06)

| # | Topic | Ruling / outcome |
|---|---|---|
| R36-a | **Owner ran β by hand and reported no problem** | `CLAUDE.md § 4` **step 5** is discharged for β. α, γ and β are now all verified by hand. Nothing is outstanding on any landed milestone |
| R36-b | **Three things he could not read, explained** | (i) *"four writers of coordination metadata"* — the point is not that a crash is designed to preserve `owner_id`, but that **nothing exists that could clear it**: all four writers are code inside an orchestrator process and `SIGKILL` runs none of them. (ii) The repeated `waiting for storage` lines in leg 5 — a bounded 30-second retry before the non-zero exit. (iii) The `orchestrators` table — a fresh row per boot, `last_seen_at` advanced every 2 s by the live process only, read in exactly two places: `§ 8.5`'s claim and `§ 5.3`'s `orphaned`-versus-`timeout` decision |
| R36-c | **A tension flagged rather than fixed: "fail fast" versus a 30-second retry** | `§ 13.1.5` says *"fail fast: non-zero exit, and an error message that names storage as the cause"* and fixes no duration. `cmd/piton/main.go` retries the first `Ping` for 30 seconds, logging once a second, then exits non-zero — with a stated reason: a container starting beside its database is not the failure case `§ 13.1.5` describes. Both observable properties hold, so this is not a violation. **Put to the owner as his call**, not changed: if 30 s is not "fast", `SPEC.md` names a bound first and the code follows (`CLAUDE.md § 5.2`) |
| R36-d | **Next milestone: δ — replay in its variants** | `SPEC.md § 18`'s order: α → γ → β → **δ**. Ratified and needed: `§ 14` in whole, `§ 10.1` (`POST /runs/{run_id}/replay`), `§ 10.5` (the 409), `§ 6.2` (`replay_count`), `§ 6.5` (`replay_round`), `§ 12.3` (which side replay resumes), `§ 5.5` (L4 and L5 are the only entry states). Missing, as before: `§ 18.4`'s demo script, which is the owner's to rule on |
| R36-e | **δ breaks the pattern of the last three milestones: it needs real code** | γ and β required no implementation because `§ 4.2`, `§ 8` and `§ 12` had to be built for α to have a correct driving loop at all. Replay is not like that: `POST /runs/{run_id}/replay` **does not exist**. `grep -rn replay internal/ cmd/` finds only `replay_count` being stored and read, and `replay_round` being stamped onto dead-letter entries — no route, no handler, no storage method. δ is therefore the first milestone since α whose step 4 is not empty, and the first where a suite derived from `SPEC.md` will legitimately fail before the implementation exists (`CLAUDE.md § 4` step 3, R20-a) |
| R36-f | **Owner is taking δ to a separate session** | Recorded so that the next session's context is not mistaken for this one's. What it needs to know is R36-d and R36-e |

---

## Round 37 — δ's environment and suite written; the suite is red, and red in the right place (2026-09-08)

| # | Topic | Ruling / outcome |
|---|---|---|
| R37-a | **Owner ruled: δ demonstrates worker-side replay only; L5 is left to ζ** | Put to him before any file was written, because the leg list depends on it. The ground is `SPEC.md § 12.1` and `§ 6.1`: the static planner never answers `fail` and *"simply cannot fail at run time… so `planner_attempt_count` never leaves 0"*, so **L5 is unreachable with the planners that exist**. The only way in today is `planner_type: "http"` pointed at a dead address, whose `error_text` names an unbuilt milestone rather than a mechanism. This is R32-a's ruling for γ, applied a second time to the same situation — and it is `CLAUDE.md § 5`'s subtractive default doing its job twice |
| R37-b | **Owner ruled: this round produces the environment and the tests only, run red** | `CLAUDE.md § 4` step 3 in its literal form — the tests exist before the implementation. Step 4 is his to call separately. R36-e predicted this is the first milestone since α where step 4 is not empty, and that prediction is now confirmed by a running system rather than by `grep` |
| R37-c | **`SPEC.md` was NOT touched, and the suite was structured so that it did not have to be** | `§ 18.4` does not exist and `CLAUDE.md § 2` rule 1 forbids writing it unsolicited. **No assertion in `test/delta` cites a demo script**; every one traces to `§ 14`, `§ 10.1`, `§ 10.5`, `§ 12.2`, `§ 12.3`, `§ 6.2`–`§ 6.5`, `§ 6.7`, `§ 5.1`, `§ 5.3`, `§ 5.5`, `§ 5.6`, `§ 4.2`, `§ 9.7` or `§ 11.1`. R34-e is the precedent, and it has the same benefit here: the suite's authority does not depend on the demo script at all. A draft `demo.sh` is written and is a proposal, not a transcription |
| R37-d | **Six legs, and why legs 3 and 4 are a matched pair** | (1) a worker-side DLQ run replayed, its step re-dispatched, the run walking on to a step that could not exist before; (2) two rounds, dying at step 1 then step 2, with step 1 never revisited; (3) eight concurrent replays, exactly one winner; (4) a replay caught **in flight**, and a second replay refused with `409 RUNNING`; (5) a `DONE` run refused; (6) an unknown `run_id` refused with 404. **§ 14 states its gate twice, the second time in the negative** — *"is this run in DLQ right now. **Not** 'has this run been replayed before'"* — and legs 3 and 4 are the two halves of that sentence. An implementation gated on `replay_count = 0` passes leg 3 and fails leg 4 |
| R37-e | **Leg 4 is the only leg that must be observed while it happens, and the fixture is shaped around that** | `§ 14` puts the replayed step back into `RUNNING`, and a step that succeeded in milliseconds would be `DONE` before any test function ran. So δ's worker sleeps 30 s **on its success path only** — round 0 still reaches DLQ promptly, and only the replayed dispatch is slow — and the reading is **one query returning seven values**, so they are consistent with one another rather than straddling the moment the attempt finished. Beta's arrangement (R34-i's lesson) applied to a different transient |
| R37-f | **`attempt_count = 0` is asserted by inference, not by catching it** | `§ 14` resets it and `§ 4.2` burns one unit at dispatch, so a reading of **1** beside a *second* attempt row can only mean the counter was put back to 0 first — had it not been, `§ 12.2`'s budget check would have refused the dispatch and that attempt would not exist. Polling for a literal 0 was rejected: the window is up to one sweep wide, so the test would pass or fail on timing, which is the flaky-assertion failure mode R34-i warns about |
| R37-g | **Three places where the suite deliberately asserts nothing** | The **success code** of a performed replay (`§ 10.5` enumerates refusal codes only, so any 2xx is accepted and the *effect in the database* is asserted instead); the **`error` slug of a 404** (`§ 10.5` prints one body in full and it is a 409); and `§ 14`'s **`CANCELLED` row** (cancellation is `§ 15` at ι, and staging a `CANCELLED` row by hand would mean the suite writing a state the product cannot reach). R34-m is the precedent for all three. Each absence is asserted rather than left silent |
| R37-h | **Two places where `SPEC.md` is thinner than the suite would like — flagged, not fixed** | (i) **`attempt_no` across rounds.** `§ 14` resets `attempt_count` and leaves the `attempts` rows in place; `§ 6.4` calls `attempt_no` *"1-based ordering within the step"* but does not say a replayed round continues the numbering. A round restarting at 1 would give one step two rows numbered 1, and the column would stop being an ordering — so the suite asserts contiguity on that reading. (ii) **`§ 6.2` calls `replay_count` "number of completed replay rounds"** while `§ 14` increments it as the replay *begins*; `§ 6.5`'s definition of `replay_round` only works if the increment comes first, so the behaviour is unambiguous and it is the word *completed* that is loose. `CLAUDE.md § 5.2`: SPEC changes first, the test second — neither was touched |
| R37-i | **Results: red, and red in exactly one place** | `./test/delta/run.sh` — the environment came up, migrations applied, the orchestrator served, and **leg 1's run reached `DLQ` under its own power**, which is what proves δ's compose file, worker, workflows and harness all work. The fixture then stopped at the first line that needed the milestone: `POST /runs/{run_id}/replay` returned **`404 page not found`** — Go's default mux answer for a route that was never registered. No test function ran, which is the correct shape for a suite whose subject does not exist yet. `gofmt`, `go build ./...`, `go vet ./...` clean; `bash -n demos/delta/demo.sh` clean; all five workflow files parse |
| R37-j | **β's environment was taken down, with the owner's permission and not without it** | It had been left up by his hand-run (`demo.sh` without `--down`) and held port 8080, so δ's environment could not start. Asked rather than assumed: it was the environment he was inspecting, and R36-a's hand-verification does not make it disposable. He agreed |
| R37-k | **`BACKLOG.md` B18's own trigger condition has been passed** | B18 said *"the third milestone that wants the same plumbing is the point at which the trade turns — two copies is duplication, three is a pattern"*. `test/delta/harness` is the **fourth** copy. The entry was updated to record that rather than left saying something that is no longer true. Not acted on: three of the four guard milestones the owner has verified by hand, which is the same reason R33-g gave for not touching them, and `BACKLOG.md` is authority on nothing. **The trade is now his to call** |
| R37-l | **Still outstanding** | `CLAUDE.md § 4` **step 4** for δ — the implementation, which does not exist; the `§ 18.4` ruling; and then step 5, the owner's hand-run |

---

## Round 38 — δ implemented; the suite is green, and two of its three failures were the suite's own (2026-09-08)

| # | Topic | Ruling / outcome |
|---|---|---|
| R38-a | **Owner directed: implement** | `CLAUDE.md § 4` step 4, against the suite round 37 had already derived from `SPEC.md` alone. This is the first milestone since α whose step 4 was not empty, as R36-e predicted |
| R38-b | **Three files changed, and nothing else** | `internal/storage/storage.go` (a new `ErrNotInDLQ` sentinel and the `ReplayRun` method), `internal/storage/postgres/postgres.go` (the transaction), `internal/httpapi/api.go` (the route, the handler, and `slugConflict`). **`internal/engine` was not touched.** `§ 14` leaves the run in `§ 5.5`'s **L2** — run `RUNNING`, last step `RUNNING`, `attempt_count` 0 — and the driving loop's existing claim path already re-dispatches from exactly there once the budget check passes. `§ 12.3`'s *"what replay resumes: re-dispatching that step"* is therefore not a new code path but the one α had to build anyway, which is the same finding as R27-f, R33-i and R34-k arriving from the other direction |
| R38-c | **The gate is one conditional `UPDATE`, and nothing outside it decides the winner** | `UPDATE runs SET … WHERE run_id = $1 AND status = 'DLQ' RETURNING replay_count`. `§ 14` says *"the transaction that takes the run out of DLQ **is** the gate"*, and this is that sentence as SQL. It is a CAS in `§ 8.1`'s sense, and it works because of `§ 7.3` obligation 3 — *"an `UPDATE` blocked by a row lock must re-evaluate its `WHERE` clause after the lock is released"* — so the second of two simultaneous replays blocks, wakes, sees `RUNNING`, affects zero rows and is refused. A `SELECT` followed by an `UPDATE` is the shape `§ 8.1` rejects outright, and was not written |
| R38-d | **No ownership fence, and why that is not an omission** | `§ 8.2`'s fence asks *"am I still this run's owner?"* and is asked by a driver about a run it drives. A replay is the operator's, and a run in DLQ has no owner to test: `§ 6.2` makes `owner_id` non-`NULL` only while `status = 'RUNNING'`, and `§ 8.7`'s fourth writer cleared it in the same transaction that wrote the DLQ verdict. `§ 14` names its own gate in its place. `§ 14`'s *"clear `owner_id` and `claimed_at`"* is still written, because `§ 14` asks for it and says why it costs nothing — *"a `DLQ` run already holds both as `NULL`, so this is a restatement for the reader, not a second mechanism"* — so it cannot become a fifth writer of coordination metadata |
| R38-e | **A tension between `§ 8.1` and `§ 14`, flagged rather than resolved** | `§ 8.1` says *"Piton has exactly **two** CAS predicates"*, and δ's gate is a third — `run_id = :rid AND status = 'DLQ'`. It is not a contradiction on the plain reading: `§ 8.1`'s sentence closes with *"everything **in this section** is one of them"*, and `§ 14` defines its own gate outside `§ 8`. But an implementer counting CAS predicates in this codebase will now find three where SPEC says two. **Put to the owner as his call** — if `§ 8.1` should acknowledge the replay gate, `SPEC.md` says so first and nothing in the code changes (`CLAUDE.md § 2` rule 1, `§ 5.2`) |
| R38-f | **First suite run: 18 passed, 3 failed — and the implementation caused none of the three** | Recorded in full because "the tests went red" and "the code is wrong" are not the same sentence, and R34-i is the standing reminder that the difference is where the danger lives |
| R38-g | **Failures 1 and 2 were invalid SQL this agent had written** | `SELECT … FROM generate_series(0, count(*) - 1)` — Postgres answers *"aggregate functions are not allowed in functions in FROM"*, so the query never ran at all. Rewritten with `row_number() OVER (PARTITION BY …)`, which asserts the identical claim: sorted within each group the values read 0, 1, 2 … with no gap (the equality breaks) and no repeat (`row_number()` is distinct where the column would not be). **The assertion's meaning is unchanged; only the SQL expressing it changed** — this is not `CLAUDE.md § 5.2`'s forbidden move, because nothing was weakened to fit observed behaviour. The same two queries were wrong in `demos/delta/demo.sh` and were fixed there too |
| R38-h | **Failure 3 was a boolean this agent had encoded wrongly, and `demo.sh` is what proved it** | `TestReplayPutsTheRunAndItsStepBackIntoRunning` reported that a replayed run was not owned. `demo.sh` asserts the same thing and **passed**, on the same environment, which located the difference precisely: a boolean read as **its own column** prints `t` under `psql -At`, while the same boolean **concatenated into a string** prints `true`. The suite packs seven values into one row — one query, so that seven readings cannot straddle the moment the attempt finished — and therefore took the second path. Verified against a live Postgres rather than deduced: `SELECT (1 IS NOT NULL)` → `t`; `SELECT 5 \|\| chr(124) \|\| (1 IS NOT NULL) \|\| chr(124) \|\| 7` → `5\|true\|7`. Fixed by emitting `owned` / `unowned` from a `CASE`, so the snapshot no longer depends on which of the two renderings applies. **The run was correctly owned throughout; the assertion still demands that it is** |
| R38-i | **Why that one is worth writing down** | It is the failure mode R34-i named, wearing the opposite face. There, a weak fixture would have passed while testing almost nothing; here, a correct implementation was reported broken by the test's own plumbing. Both are cases where the green/red signal says something other than what it appears to, and both were only caught because a second, independent path existed — the hand-run demo (`CLAUDE.md § 4` step 5) checking the automated suite rather than the other way round |
| R38-j | **Results** | `./test/delta/run.sh` — **21 tests and 9 subtests, all pass, 72 s**, legs ending `DONE / DONE / DONE / DONE / DONE`, five runs and five dead-letter entries that survived every replay. `demos/delta/demo.sh` — **51 assertions, 0 failures**. `gofmt`, `go build ./...`, `go vet ./...` clean. No container or volume left behind |
| R38-k | **Two separate execute-bit facts, and only one of them was real** | (i) `demos/delta/demo.sh` genuinely lost its `+x` when this agent edited it from the Windows side — the edit rewrites the file and drops the mode — and the next run failed with `Permission denied`, which at first glance is indistinguishable from a demo that fails. Restored with `chmod +x` inside WSL. (ii) The six pre-existing `M demos/*/demo.sh` and `M test/*/run.sh` entries were **not** real changes: `git` run from the **Windows** side over the `\\wsl.localhost` UNC path cannot see ext4 permission bits and reports every file as `100644`. Run from inside WSL, `git status` shows those six files unmodified. Recorded because the first reading — "something stripped the execute bit off six landed scripts" — was wrong, and because it is one more instance of `CLAUDE.md § 8`'s rule earning its place: **the Windows side is an editor only, and that now demonstrably includes `git status`** |
| R38-l | **Still outstanding** | `CLAUDE.md § 4` **step 5** for δ — the owner's own hand-run — and the `§ 18.4` ruling. R38-e is his call too, and the B18 question R37-k raised is unchanged |
| R38-m | **Dead code this agent had introduced, removed after the green** | Four things existed that nothing used: `harness.RunDigest` (leg 1's append-only assertion is served by the narrower `DLQDigest`), `harness.SweepInterval` and its `configSeconds` reader (β needs those because its waits are bounded by a lease that must expire; every δ wait polls for the state itself, so there is nothing to keep in step with `piton.yaml`), and `demo.sh`'s `replay_body` shell function. Removed rather than left, per `CLAUDE.md § 5`'s subtractive default — unused scaffolding in a test harness reads as a capability the suite has and does not. `Scalar` and `Int` in `assert.go` were **kept** although δ uses neither: they are part of the assertion vocabulary the other three harnesses share, and diverging this copy would make B18's eventual merge harder rather than easier |

---

## Round 39 — δ re-verified after a dead-code sweep; the owner is directed to the hand-run (2026-09-08)

| # | Topic | Ruling / outcome |
|---|---|---|
| R39-a | **Owner asked for replies in Chinese** | Recorded as a standing preference. It covers conversation prose only: everything written into the repository stays English under `CLAUDE.md § 7`, so `SPEC.md`, this log, code comments, test names and `demo.sh` output are unaffected |
| R39-b | **Dead code δ had accumulated is gone from the tree** | `harness.RunDigest`, `harness.SweepInterval`, `harness.configSeconds` (and with them the `regexp` / `strconv` imports), the stranded `SlugNotFound` doc comment, and `demo.sh`'s unused `replay_body`. All were introduced by this agent in round 37 and never used by any assertion. **These edits were not made by this agent in this session** — they were found already applied on disk — so the removal is recorded as a fact about the tree rather than claimed as work done here |
| R39-c | **The suite was re-run before the owner was pointed at the hand-run, and that is a rule rather than caution** | `CLAUDE.md § 5.4`: tests run "always before the owner is asked to look at a milestone by hand". Files had changed after the last green run, so the previous green no longer described the tree. **21 tests, 9 subtests, all pass, 80 s**; `gofmt`, `go build ./...`, `go vet ./...` clean; no container or volume left behind |
| R39-d | **Owner asked which of the two he should run by hand** | Answered: the **demo**, not the suite. `CLAUDE.md § 4` step 5 is his and is "not replaceable by step 6", and `SPEC.md § 17.4` is the sentence behind it — "a green suite the owner has never seen behind is not evidence that a milestone landed". Round 38 had just demonstrated the same point from the other direction: it was `demo.sh` passing while the suite failed that located R38-h's defect in the suite's own plumbing |
| R39-e | **What he was told to look at, and why those three** | (i) `attempt_count = 1` beside three `attempts` rows — the divergence `§ 6.3` exists to permit, and the reason that column is stored rather than derived; (ii) leg 2's two entries, `replay_round` 0 and 1 naming different steps, with the first step untouched by the second round — `§ 14`'s accepted limitation seen rather than asserted; (iii) five `DONE` runs beside five surviving dead-letter entries — `§ 6.7`'s append-only rule at the one moment something would be tempted to tidy the entry away, since what it recorded is no longer true |
| R39-f | **Still outstanding** | `CLAUDE.md § 4` **step 5** for δ; the `§ 18.4` ruling; R38-e (whether `§ 8.1`'s "exactly two CAS predicates" should acknowledge the replay gate); and R37-k (B18's four harness copies) |

---

## Round 40 — δ committed and pushed; ζ scoped; `fetch_base_url` given a home and transcribed (2026-09-09)

| # | Topic | Ruling / outcome |
|---|---|---|
| R40-a | **δ is closed, committed and pushed** | The owner reported δ complete, which discharges `CLAUDE.md § 4` **step 5** — the item carried since R38-l and R39-f. Committed as `dee2531`: the implementation, `demos/delta/`, `test/delta/`, rounds 37–39 and `BACKLOG.md`'s B18 update, in one commit, then pushed to `origin/main` |
| R40-b | **R38-k's rule earned its place a second time** | A Windows-side `git status` again reported six modified `demos/*/demo.sh` and `test/*/run.sh` files. From inside WSL the tree held five modified files and two untracked directories and nothing else. The UNC path cannot see ext4 permission bits, so **every** git operation in this round was run through `wsl.exe`, not from the Windows side |
| R40-c | **Next milestone: ζ — a planner that is an external HTTP service** | Until now "which step comes next" is a list fixed at `POST /workflows` and nothing can change its mind. ζ makes the decision a synchronous HTTP call to a service the user owns, which may be an LLM. `SPEC.md § 18` puts it fifth for a stated reason: it is the first milestone that makes the system useful to somebody other than its author |
| R40-d | **ζ needs real code, and more of it than δ did** | `internal/planner` holds the static planner only, and `internal/engine` answers an `http` workflow with *"milestone zeta and is not implemented in this build"*, reported as a planner call that could not be made — so the run burns planner budget and converges to DLQ under `§ 12.2` rather than hanging. What is missing is a planner client: M1 out (`§ 9.2`), M2 parsed (`§ 9.3`), `planner_timeout_seconds`, the budget path, and the four `planner_*` dead-letter reasons of `§ 6.5` |
| R40-e | **Owner ruled: tests are written from `SPEC.md`, and a demo script is never a source** | He asked what "a test citing a demo script" even meant. Answered: a demo script is written by this agent, so a test derived from one is this agent checking its own homework — `CLAUDE.md § 5.1` in plain words. His answer settles ζ the way R37-c settled δ: `test/zeta` will trace every assertion to a ratified section, and `demos/zeta/demo.sh` is a proposal until he rules on it |
| R40-f | **Owner ruled: "switching between planners" means the choice made at workflow creation, and nothing else** | `SPEC.md § 18`'s ζ row says *"and switching between planners"*, and no section anywhere defines it — `§ 10.1` has no endpoint that modifies a workflow. The reading is now fixed: **you choose `static` or `http` when you create the workflow. That is it.** No new endpoint, no new wire shape. ζ's demo shows one `static` workflow and one `http` workflow over the same workers, both reaching `DONE` — the engine not caring who decides |
| R40-g | **Owner ruled: no real LLM inside the automated suite** | An LLM call is non-deterministic, needs a key and needs the network, and a suite that answers differently on different days guarantees nothing. If a live LLM is wanted it is a manual leg outside the suite |
| R40-h | **How a dynamic planner is proved rather than merely shown** | The owner's own shape: a first worker produces something, and the planner picks the second worker from it. The fixture that makes it an argument: the `classify` worker **alternates** its answer between two values from its own internal state, and **two runs are started with identical input**. The planner receives a catalogue that carries `output_bytes` and no content (`§ 9.2`), so to branch at all it must fetch the output through the read API (`§ 10.2`). Two runs with identical input taking different paths is therefore proof that the fetch happened — with no fetch the planner holds nothing that differs between them — and a static planner cannot produce it by construction. A second, direct reading is available too: the demo planner is our own fixture and can expose a count of what it fetched |
| R40-i | **A gap in `SPEC.md` found before any file was written: `fetch_base_url` had no source** | `§ 9.2` makes the field **required** in every M1 — it is how the planner is told where to read the outputs the catalogue omits — but `§ 4.4`'s list of what the configuration file must declare does not mention it, `internal/config` has no such key, and the string appears nowhere in the tree. ζ cannot be built without deciding where it comes from, and "the orchestrator's own externally visible address" is a definition, so `CLAUDE.md § 9` forbade choosing one silently |
| R40-j | **Three candidate homes were put to the owner, with the cost of each** | (甲) the operator supplies it at `POST /workflows`, required iff `planner_type = 'http'`; (乙) the orchestrator's configuration file holds it and the value is stamped onto the workflow row at creation; (丙) the configuration file holds it and it is read at each planner call. A fourth possibility — dropping the field and letting each planner know its own orchestrator — was analysed and rejected in the same exchange: it does not remove the network requirement, only moves who holds the string, and its failure mode is a planner pointed at another deployment, where reads **succeed** against the wrong database and produce a wrong decision instead of an error |
| R40-k | **Owner ruled: 甲** | Two reasons recorded. (i) It is one half of a pair: `planner_url` is where the orchestrator calls this workflow's planner and `fetch_base_url` is where that planner reads back — one relationship, two directions, settled at the same moment, in the same row, expiring together. (ii) A single process-wide value cannot give two workflows two different routes to the same orchestrator, and a planner inside the cluster and one hosted outside it do not reach the read API by the same address |
| R40-l | **The cost of 甲, accepted with his eyes open** | `§ 10.1` has no endpoint that modifies a workflow (R40-f), so a deployment that changes its externally visible address strands every existing `http` workflow — including the **replay** of a run sitting in DLQ under one, which re-asks the planner and hands it an address that no longer answers. The remedy is a new workflow. This is **not a new class of cost**: `planner_url` is already immutable in the same way and fails the same way when a planner moves, so 甲 adds no concept — whereas 丙 would have made one address of the pair mutable and the other not |
| R40-m | **`SPEC.md` transcribed on the owner's direction — four places** | `CLAUDE.md § 2` rule 1: he ruled, and the edits were put to him before they were made. (i) `§ 6.1` gains the `fetch_base_url` column, *"required iff `planner_type = 'http'`; must be absent otherwise"*, placed beside `planner_url`; (ii) `§ 6.1`'s invariant sentence now names both `http` columns together; (iii) a **Why** paragraph in `§ 6.1` records R40-k's reasons and R40-l's accepted cost in full; (iv) `§ 16` rule 2 requires it and rule 3 rejects it under `static`; (v) `§ 9.2`'s field table says the value is the workflow's column, verbatim. **`§ 4.4`'s configuration list was NOT touched** — 甲 makes it moot, and the question of adding a line to it, raised earlier in the same exchange, died with the ruling |
| R40-n | **Rule numbering in `§ 16` was extended, never renumbered** | `internal/validate/validate.go` cites *"SPEC.md 16 rule 2"* in its error text, so inserting a new numbered item would have silently renamed a rule the code points at. Rules 2 and 3 were widened instead |
| R40-o | **Why the schema change is a new migration and not an edit to the existing one** | Asked by the owner, because "the applied migration may not be edited" and "a column is being added" sounded contradictory. `golang-migrate` decides what to run **by version number, not by content**: a database that has recorded version 1 will never re-read file 1, so editing it leaves existing databases without the column while fresh ones get it — two databases both calling themselves version 1 with different shapes. A new `000002` is an instruction every database runs exactly once, so all of them converge. Not academic here: R37-j's β environment survived a session, so long-lived databases on this machine are a thing that happens |
| R40-p | **What ζ inherits, twice deferred** | The planner-side dead-letter demonstration, dropped from γ by R32-d and from δ by R37-a because the static planner cannot fail. It is due now: the four `planner_*` reasons of `§ 6.5`, the **L5** combination of `§ 5.5`, and `§ 14`'s replay of an L5 run — which resumes by **asking the planner again**, not by re-dispatching a step, and is the other column of `§ 12.3` |
| R40-q | **Still outstanding** | `§ 18.4` — δ's demo script, never ruled on; `§ 18.5` — ζ's, which does not exist and will be handled as δ's was (R37-c); R38-e — whether `§ 8.1`'s *"exactly two CAS predicates"* should acknowledge the replay gate; R37-k — B18's four harness copies. None is blocking ζ |

---

## Round 41 — ζ's environment and suite written; the suite is red, and red in exactly one place (2026-09-09)

| # | Topic | Ruling / outcome |
|---|---|---|
| R41-a | **Owner ruled the eight legs, then directed: write the suite** | The leg list was put to him before any file was written, as R37-a's precedent requires: (1) the dynamic branch, (2) a `static` workflow beside the `http` one, (3) an unreachable planner, (4) an unusable answer, (5) mixed failures, (6) a declared `fail`, (7) replay of a planner-side dead letter, (8) the submission-time refusals. He approved all eight without cutting any |
| R41-b | **Owner ruled: heavy-but-easy work may go to Sonnet, but the review must be this agent's own** | Two subagents were used. One wrote `demos/zeta/` — the compose environment with its two inline-Python fixtures, `piton.yaml`, and ten workflow files. One wrote `test/zeta/harness/`, `run.sh` and `README.md`, a port of delta's |
| R41-c | **What was NOT delegated, and why the line falls there** | The assertions. `CLAUDE.md § 5.1` makes `SPEC.md` the only legitimate source of a test, and an agent that has not read the ratified sections cannot derive one from them — it would be writing from a brief, and the brief is this agent's summary, which is the circularity `§ 5.1` exists to forbid. `test/zeta/planner/main_test.go` and `planner_test.go` were written here, against `SPEC.md` directly |
| R41-d | **The fixture contract was fixed in the brief before either subagent started** | Paths, modes, ports, the `/stats` shape, the ten fixtures and their `planner_max_attempts` were all named up front, because the assertions and the fixtures are two halves of one thing and a subagent inventing either half would have made them disagree. This is why two agents could work in parallel on files that must fit together |
| R41-e | **A defect in this agent's own brief, caught mid-flight and corrected** | The brief told the demo planner to read the classify verdict at `body["kind"]`. `SPEC.md § 6.3` stores `steps.output` as *"the worker's whole response body, verbatim"* and `§ 9.6`'s sync+envelope success shape wraps it, so the verdict actually sits at `body["output"]["kind"]`. Left uncorrected, the planner would have found no `kind`, answered `fail`, and leg 1 would have failed for a reason belonging to the fixture rather than to Piton — R38-h's failure mode arriving from a third direction. Corrected by message while the agent was still working, and the agent also made the parse defensive so a wrong shape shows as a named failure instead of a silent wrong branch |
| R41-f | **The subagent found a defect in this agent's file, and correctly refused to fix it** | `go vet` over the whole of `test/zeta` failed on `non-name obs.controlCode on left side of :=` in `main_test.go`. It was told not to write test files, so it reported the error and left it alone rather than helpfully editing a file outside its brief. Fixed here |
| R41-g | **Why leg 1 is a proof rather than a demonstration** | Two runs are started **from one workflow row with one input**, and the `classify` worker answers `invoice` to the first and `receipt` to the second from its own ordered registry — an answer that is stable under retry, because it looks the run up rather than advancing a counter. `SPEC.md § 9.2`'s catalogue carries `output_bytes` and no content, so **nothing in the request tells the two runs apart**: a planner that did not fetch holds one question twice and cannot answer it two ways. The differing second step is therefore evidence that the fetch happened, and `§ 6.1`'s static planner cannot produce the shape at all — its answer is a function of the step COUNT. A second, direct reading exists too: the fixture's own `/stats` counter, which the database cannot show |
| R41-h | **Leg 8 carries a control body, and it is not decoration** | Each refused body differs from an **accepted** one in exactly one field, and the unmutated fixture is submitted alongside them. Without that, a body rejected for an unrelated reason produces a green test that proves nothing. The same defect was found earlier in this exchange in `test/alpha/validation` and is recorded at R41-m |
| R41-i | **One subtest beyond the eight legs, named rather than slipped in** | Leg 3 gained a third case: a planner that sleeps five seconds against a workflow whose `planner_timeout_seconds` is 1. The ground is `§ 6.5`, which deliberately gives one reason to three events — *"connection refused, timeout, non-2xx"* — so a suite with no slow planner never exercises `§ 11.1`'s `planner_timeout_seconds` at all, and a configuration field that silently does nothing is exactly what `§ 16`'s principle is against. **It is the owner's to cut** under `CLAUDE.md § 5`'s subtractive default |
| R41-j | **Three places where the suite deliberately asserts nothing** | (i) **How many times the planner was asked for one decision.** `§ 13.2` item 4 publishes the opposite as a non-guarantee — *"the 'asked exactly once' guarantee covers persisted decisions only"* — so the suite asserts the persisted decisions and the direction of the fixture's counter, never an exact call count. (ii) **`last_planner_error` after a declared `fail`.** `§ 6.2` defines the column as the text of the most recent **failed** call and `§ 12.1` says a `fail` answer is not one; SPEC is silent about the column in that case, so nothing is asserted. (iii) The success code of a performed replay, as R37-g settled |
| R41-k | **A tension in `§ 6.5` found while deriving leg 5, flagged and not resolved** | `planner_budget_exhausted` is *"set instead of the two above when the round's failures were not all of one kind"* — but **no column persists the kind of each failure in a round.** `runs` holds a count and the most recent error text, and nothing else. An implementation must therefore carry the kinds in memory for the length of a round, and a crash mid-round loses them, so the entry could name a different reason than an uninterrupted round would have. The consequence is confined to a **label on one dead-letter entry** and never to state, so it does not break `§ 1`; but `§ 1` says no in-memory mechanism may be load-bearing, and this sits on the boundary. **Put to the owner as his call**, changing nothing: if the reason must survive a crash, `SPEC.md` says so first and a column follows |
| R41-l | **Results: red, and red in exactly one place** | `./test/zeta/run.sh` — the environment came up, migrations applied, the orchestrator registered and served. The fixture then stopped at the first line that needs the milestone: `POST /workflows` refused every `http` fixture with `unknown field "fetch_base_url"` — the validator has not learned the amendment this same day's round 40 transcribed, and the migration that adds the column does not exist. **No test function ran**, which is the correct shape for a suite whose subject does not exist yet (`CLAUDE.md § 4` step 3, R37-i's precedent). `gofmt`, `go build ./...`, `go vet ./test/zeta/...` clean; ten fixtures parse; `docker compose config` valid; both inline Python programs compile; no container and no volume left behind |
| R41-m | **A weakness found in `test/alpha/validation`, before the regression sweep has begun** | Two of its cases submit an `http` workflow to prove `planner_url` is validated. After the `§ 6.1` amendment they are **still rejected — for the wrong reason**: the body now also lacks `fetch_base_url`. They stay green while testing nothing, and would stay green if `planner_url` validation were deleted outright. The fix is to give those bodies a valid `fetch_base_url` so only the defect under test remains. It follows `CLAUDE.md § 5.2`'s permitted direction — SPEC changed first, by the owner's ruling, and the test follows — never the forbidden one |
| R41-n | **Owner has ordered a regression sweep of every earlier suite against today's SPEC amendment** | α, γ, β and δ are all static-planner suites, so the new column cannot make a valid workflow invalid, and no test anywhere asserts the shape of a workflow read, and no demo script does `SELECT *` on `workflows`. The blast radius is R41-m and nothing else found so far. The sweep runs for real once ζ's implementation lands, because until then the schema and the validator have not changed at all |
| R41-o | **Still outstanding** | `demos/zeta/demo.sh` — not yet written; ζ's implementation (`CLAUDE.md § 4` step 4) and then the owner's hand-run (step 5); `§ 18.5`; R41-i and R41-k, both his calls; and R40-q's carry-overs — `§ 18.4`, R38-e and R37-k |

---

## Round 42 — ζ's demo script written and reviewed; the `§ 6.5` question explained in full (2026-09-09)

| # | Topic | Ruling / outcome |
|---|---|---|
| R42-a | **Owner ruled: write the demo script now, before the implementation** | `CLAUDE.md § 4` puts step 2 (the demo script) before step 4 (implement), and he said so in his own words — the demo should be written before the code. `demos/zeta/demo.sh` now exists: 728 lines, eight legs, executable |
| R42-b | **Owner ruled: do not commit yet** | Nothing from rounds 41 or 42 is committed. `SPEC.md`'s amendment and round 40 are (commit `10dccee`, unpushed) |
| R42-c | **The demo was delegated; the review was not** | Written by a Sonnet subagent against a brief that fixed the eight legs, then reviewed here line by line: every SQL query read for validity, every fixture reference checked against the directory (ten referenced, ten present, none unused), `bash -n` clean, the execute bit set from inside WSL. **It cannot be run**, because ζ's implementation does not exist — the same position delta's demo was in at R37-i |
| R42-d | **What the review confirmed, and what it could not** | Confirmed: no assertion cites a demo script or a test file; every citation names a ratified section; leg 8 submits the unmutated body as a control; no query repeats R38-g's aggregate-in-`FROM` defect; and the script asserts only the **direction** of the planner fixture's call counter, never an exact count, because `§ 13.2` item 4 publishes the opposite as a non-guarantee. Not confirmable until the implementation lands: that the assertions actually hold |
| R42-e | **Two deliberately brittle assertions, named rather than hidden** | The script asserts `count(*) = 11` on `workflows`, `= 11` on `runs` and `= 8` on `dead_letter_queue`. Adding a leg later breaks them by design — they are what makes "a refused workflow is not stored" an assertion about the whole database rather than about one row |
| R42-f | **Owner asked what the `§ 6.5` problem actually is; explained in full** | Three questions: what a *round* is, what a *kind* is, and at which moment the crash happens. (i) **Kind** is `planner_unreachable` versus `planner_invalid_response` — the two classes of planner failure, which exist as separate values because they call for different actions from the operator. (ii) **Round** is the run of consecutive failed calls that burned one budget at one decision point; `§ 6.2` resets the counter on any successful call. (iii) The crash is the **orchestrator process dying after some budget has been burned but before the dead-letter entry is written**: a successor reads `planner_attempt_count` and `last_planner_error` from the database, which record *how many* and *the latest text*, never *which kind each was* — so it cannot tell whether the round's failures were all of one kind, and may label the entry differently from an uninterrupted run |
| R42-g | **A second ambiguity in the same sentence, found while explaining it** | `§ 6.5` says *"the round's failures"*, and `§ 6.5` also defines `replay_round` as the replay counter. Read strictly, *round* would mean the replay round and would include failures at earlier decision points that a later success already forgave. The sensible reading is the budget-burning run of failures. **The suite is safe under both readings** — its mixed fixture has one decision point inside one replay round — so no test is pinned to either |
| R42-h | **Three ways out were put to the owner, with a recommendation** | (1) persist each failure's kind, so the label survives a crash — costs a write per planner failure that serves only a label; (2) state in `SPEC.md` that the reason is decided from the failures the current process observed and that a crash may change it, accepted and documented — costs one sentence and no code; (3) drop the "not all of one kind" rule and let the reason follow the last failure — costs the ability to tell a consistently unreachable planner from a mixed round. **Recommended (2)**, on the ground that nothing reads the label to make a decision. Undecided; `SPEC.md` untouched |
| R42-i | **Still outstanding** | ζ's implementation (`CLAUDE.md § 4` step 4) and the owner's hand-run (step 5); `§ 18.5`; R42-h and R41-i, both his calls; the regression sweep of the earlier suites, which cannot run until the schema and validator change (R41-m, R41-n); the commit of rounds 41–42; and R40-q's carry-overs — `§ 18.4`, R38-e and R37-k |

---

## Round 43 — the planner call becomes an entity; `SPEC.md` amended in eleven places (2026-09-09)

| # | Topic | Ruling / outcome |
|---|---|---|
| R43-a | **The owner's opening question, and the fact that corrected it** | He asked whether attempts store a failure reason. **They do** — `§ 5.3` gives six values and `§ 6.4` makes "`FAILED` ⇒ a reason is present" a backend-enforced invariant. What has no row at all is the **planner call**: `runs` held a count and the newest error text, and nothing recorded what each individual call did |
| R43-b | **His observation, which is exact** | On the worker side `dead_letter_queue.reason` has exactly one possible value, `worker_budget_exhausted`. It is a constant, so it carries no information; everything informative is already on the `attempts` rows, reachable by `step_id`. Dropping it there costs nothing |
| R43-c | **Why the same move could not simply be made on the planner side** | The four `planner_*` values were the **only** record of what kind of failure had happened, precisely because planner calls had no rows. Deleting the column without replacing it would have turned the planner side into a blank |
| R43-d | **Owner ruled: align the two sides — planner calls get rows and become an entity** | This is the change that dissolves R41-k and R42-h rather than answering them. The "was this set of failures all of one kind" rule existed only because an entry had to summarise several failures in one word; with a row per call there is nothing to summarise, the rule is deleted, and the crash that used to change the label cannot |
| R43-e | **His own four-way list was right; it belonged on a different table** | He proposed *planner timeout / planner network error / planner returns fail / planner other*. Three of those are failures and are now `planner_calls.failure_reason` — and they are **the same three names `§ 5.3` already uses for an attempt**, with the same meanings and the same clock rule. The fourth is not a failure at all and stays on the dead-letter entry |
| R43-f | **Owner asked why the dead-letter entry recorded the budget consumed, and the answer produced the last piece** | It was the only surviving record: `§ 14` resets `steps.attempt_count`, so after a replay the live counter no longer says what the failed round burned, and the `attempts` rows carried nothing saying which round they belonged to. He ruled the column out and ruled `replay_round` onto every attempt and every planner call instead — the fact moves onto the row it is a fact about, and "how much did round *n* burn" becomes a count |
| R43-g | **Owner ruled against a single shared timeout/attempts setting** | Asked as an option; declined. `§ 11.1` already carries symmetric `step_*` / `planner_*` families for a stated reason — the previous project's `retry_limit`, `RETRY_MAX_ATTEMPTS` and a hard-coded 30 seconds, where nobody could tell which governed what. Collapsing them would re-create exactly that, and the two families legitimately hold different numbers |
| R43-h | **The symmetry was completed rather than reduced** | `planner_retry_delay_seconds` is added, default 0, matching `step_retry_delay_seconds`. It closes a real gap: the code has been pausing a hard-coded **one second** between planner calls, a value `SPEC.md` never mentioned, the operator cannot see and cannot change. `§ 11.1` now says a hard-coded pause is not a substitute for the field |
| R43-i | **Owner directed: commit first, then change `SPEC.md` first** | The pre-change state is commit `d88d576` — ζ's environment, suite and demo as they stood against the old design, so that what is being replaced is on the record. The amendment is a separate commit |
| R43-j | **Eleven places amended, and not one section renumbered** | `§ 3.2` (the entity and the containment sentence), `§ 3.3` (`planner_call_id`), **new `§ 5.8`** (planner call states and the three shared failure reasons), `§ 6.1` (the new configuration column), `§ 6.2` (`last_planner_error` removed; why the counter stays), `§ 6.4` (`replay_round`; `attempt_no` stays contiguous), `§ 6.5` (five reasons cut to three; `attempt_count` removed), `§ 6.7` (table discipline), **new `§ 6.8`** (`planner_calls`), `§ 7.2` (its index), `§ 11.1`, `§ 12.1`, `§ 12.2`, `§ 12.3`, `§ 12.4`, `§ 14`, `§ 16` rule 6, `§ 17.1`, `§ 18.2`'s ratified queries, and Appendix A. **New sections were appended as `§ 5.8` and `§ 6.8` rather than inserted**, because `internal/validate` cites *"SPEC.md 16 rule 2"* by number and four milestones' suites cite `§ 5.4`–`§ 5.7` and `§ 6.5`–`§ 6.7`; inserting would have silently renamed rules the code points at (R40-n's precedent, applied to sections rather than list items) |
| R43-k | **`§ 18.2` — a ratified demo script — had to change, and this is recorded rather than glossed** | γ's script queries `dead_letter_queue.attempt_count`, a column that no longer exists. Its query now reads the count off the `attempts` rows carrying `replay_round = 0`, which is the same number from the table that owns it. The owner has hand-verified γ (R34-a), so this is a change to something he has already seen work |
| R43-l | **What is now false in the tree, which is the whole regression sweep he ordered at round 41** | The schema (no `planner_calls`, no `replay_round`, a `last_planner_error`, a `dead_letter_queue.attempt_count`); `internal/engine`'s hard-coded pause; `internal/httpapi`'s run response, which exposes `last_planner_error`; γ, β and δ's suites and demo scripts, which assert `worker_budget_exhausted` and fold `attempt_count` into their dead-letter digests; and the whole of ζ's suite, which asserts four planner reasons that no longer exist. **ζ's is the cheapest to rewrite and was written knowing this was coming** |
| R43-m | **Two things deliberately not added** | A read endpoint for a run's planner calls — `§ 10.2` is an API surface and `CLAUDE.md § 3` sends an extra endpoint to `BACKLOG.md`, while `§ 17.1` already makes the rows visible at a terminal. And a link from a planner call to the step it created — it would mean writing to a row `§ 6.7` has just called append-only |
| R43-n | **Still outstanding** | The migration and the implementation; the rewrite of ζ's suite and demo against the amended `SPEC.md`; the γ / β / δ sweep; `§ 18.5`; R41-i (the slow-planner subtest, his to cut); and R40-q's carry-overs — `§ 18.4`, R38-e and R37-k |

---

## Round 44 — the amendment implemented; four suites green, and a contradiction inside `SPEC.md` found by the fifth (2026-09-10)

| # | Topic | Ruling / outcome |
|---|---|---|
| R44-a | **Owner directed the order, and it was followed** | Commit first, then `SPEC.md`, then the migration, then the tests, then the code. Commits: `d88d576` (ζ as it stood against the old design), `e9a5e9e` (the amendment and round 43) |
| R44-b | **Migration `000002` verified against a real Postgres, not merely written** | It applies, every `§ 6.8` invariant is enforced by a constraint that was tested by trying to violate it, and it rolls back and re-applies. Checked by refusal as well as by acceptance: a planner call carrying `worker_error`, a `DONE` call answering `fail` with no `error_text`, a `FAILED` call carrying an answer, two calls sharing a `call_no`, a dead-letter entry using one of the two reasons the amendment deleted — each was rejected |
| R44-c | **A migration that cannot run, made to say so** | A database holding an `http` workflow written before today cannot satisfy `§ 6.1`'s amended invariant, and no address can be invented for it. The migration refuses first, naming the rows and what to do, rather than surfacing as a bare check-constraint violation. Every environment that starts from a clean database — which is all of them (`CLAUDE.md § 5.5.2`) — passes it silently |
| R44-d | **`GET /steps/{step_id}/output` did not exist, and ζ cannot work without it** | `§ 10.2` lists it and says why it is needed early: "a planner must be able to fetch what `§ 9.2`'s catalogue cap omits". Nothing had built it, and the milestone whose central claim is *the planner fetched the output* had no endpoint to fetch from. Added, with `§ 10.5`'s 409 for a step that exists but has not completed, because `§ 6.3` forbids treating the presence of an output as completion |
| R44-e | **The engine's planner path is now one path for both planner types** | `§ 12.1` requires it — "no exemption" — so the type decides only how the decision is obtained, and timing, classification, recording and the budget are shared. The hard-coded one-second pause is gone, replaced by `§ 11.1`'s `planner_retry_delay_seconds` |
| R44-f | **A call interrupted by shutdown is not a planner failure** | `planner.Failure` carries an unexported `interrupted` flag, and the engine records nothing when it is set: burning budget because this process was asked to stop would move a run towards DLQ for something the planner did not do. `§ 13.2` item 4 already says what happens instead |
| R44-g | **A defect this agent caused, recorded rather than glossed** | A batch-edit script passed an invalid `newline` argument to `open()`, which raised **after** the file had been truncated: `internal/httpapi/api.go` was left empty, and an alpha run that happened to be in flight failed on a Docker build that could not compile it. Restored from git and reapplied. The lesson is the one `CLAUDE.md § 8` keeps teaching from a different direction — a tool that writes files is a tool that can destroy them, and "the script printed ok" is not the same as "the file is intact" |
| R44-h | **Results: α, β, δ green; γ green after one fix; ζ red** | γ still asserted `dead_letter_queue.attempt_count`, in a query split across lines that the earlier sweep's grep did not catch. Rewritten to count the attempts carrying that entry's `replay_round`, which is the same claim asked of the table that now owns it (`§ 6.5`) |
| R44-i | **ζ's failure is not a defect in the milestone: the mechanism worked end to end** | Driven by hand: the run was claimed, the planner was called, it answered `continue`, the classify worker ran, the planner was called again, **it fetched the step's output through the read API**, and every one of those exchanges is a row in `planner_calls`. It then answered `fail`, because the document it fetched was not the shape it expected |
| R44-j | **What it found is a contradiction inside `SPEC.md`, about what `steps.output` holds in envelope mode** | `§ 9.6`'s per-mode table gives sync+envelope the response `{"status":"success","output":{ }}` and says of sync+**raw** that "the **entire** response body verbatim is the output" — a distinction that means nothing unless envelope mode stores the `output` field alone. `§ 6.3` and `§ 6.4` say the opposite in general terms: "the worker's whole response body, verbatim", and `§ 6.3`'s why-clause goes further — "selecting a field out of a worker's response is the planner's job, not the engine's" — which is precisely what unwrapping the envelope does |
| R44-k | **Which reading is already in force, and the evidence for it** | The implementation follows `§ 9.6` and has since α; `internal/dispatch` cites it in a comment. **`test/alpha`, derived from `SPEC.md` and hand-verified by the owner (R34-a), asserts the same reading**: it reads `output -> 'echo' -> …`, which only exists if the envelope was unwrapped. And `§ 9.5`'s `inputs` map — "a map from `step_id` to that step's stored output" — would otherwise hand the next worker Piton's own protocol wrapper as its input |
| R44-l | **Put to the owner, unresolved, nothing changed** | `CLAUDE.md § 9`: a test derived from `SPEC.md` failed in a way that suggests `SPEC.md` itself is wrong, so this agent stops. `CLAUDE.md § 5.2` forbids the alternative of quietly editing the test to match the code. Both the ζ suite and the ζ demo were written from `§ 6.3`'s wording, as was the demo planner fixture; one ruling settles all three |
| R44-m | **Still outstanding** | The `§ 6.3` / `§ 9.6` ruling and whatever follows it; ζ's suite re-run; `§ 18.5`; R41-i (the slow-planner subtest); and R40-q's carry-overs |

---

## Round 45 — the `§ 6.3` / `§ 9.6` contradiction ruled; all five suites green (2026-09-10)

| # | Topic | Ruling / outcome |
|---|---|---|
| R45-a | **Owner ruled: the envelope is unwrapped — a step's stored output is the worker's RESULT, not the whole reply** | Put to him as two readings with their costs. He chose the one already in force: the implementation has done it since α, `internal/dispatch` cites `§ 9.6` for it, and `test/alpha` — derived from `SPEC.md` and hand-verified (R34-a) — asserts `output -> 'echo' -> …`, a path that exists only if the envelope was unwrapped. The alternative would have rewritten behaviour under three milestones he has already verified |
| R45-b | **`SPEC.md` amended in three places, and the rule moved from implied to stated** | `§ 6.3` and `§ 6.4`'s columns now read *"the worker's output as `§ 9.6` defines it for this mode, verbatim"*. `§ 6.3`'s why-clause is rewritten: Piton does not shape a worker's **result**, and unwrapping its own envelope is not shaping. **`§ 9.6` gained the rule itself** — envelope mode stores the response's `output` field, raw mode stores the entire body — rather than leaving it to be inferred from the word *entire* in one table cell, which is what made this ambiguous in the first place |
| R45-c | **Why the two modes differ, now written down** | `status` and `output` are Piton's protocol, not the worker's data. Storing the envelope whole would put this system's wrapper inside the user's result — where `§ 9.5` would hand it to the next worker as `inputs`, and every planner reading an output would strip a layer Piton put there. A raw worker was never told about Piton, so its whole reply is the result |
| R45-d | **Three readers changed, none of them the engine** | ζ's suite, ζ's demo, and the demo planner fixture each read one level too deep. The engine was right all along, which is why the ruling cost no behaviour change |
| R45-e | **Two stale references the first sweep missed, and how each was missed** | γ's `TestDeadLetterEntry` still queried `dead_letter_queue.attempt_count` — the grep missed it because the SQL was split across two Go string literals. `test/zeta/harness`'s dead-letter digest still concatenated the same column — missed because the harness exists in **four copies** (`BACKLOG.md` B18, R37-k), and only δ's was corrected. Both now count the attempts carrying that entry's `replay_round` instead |
| R45-f | **A silent failure of this agent's own tooling, worth recording** | The first γ fix never landed: a heredoc passed through `wsl.exe -- bash -lc '…'` was consumed before `python3` saw it, so the script read empty input, did nothing, exited 0, and the `&&` chain continued to a passing `go vet`. **"The command exited 0" was mistaken for "the edit was applied"**, and the next suite run was the only thing that caught it. Every subsequent edit was verified by reading the file back. This is R44-g's lesson from the other side: a tool that reports success has not thereby done anything |
| R45-g | **Results: five suites, all green** | α 20.5 s, γ 31.6 s, δ 59.1 s, β 103.3 s, ζ 41.9 s. `gofmt`, `go build ./...`, `go vet ./...` clean. Two anonymous Docker volumes left by this agent's own throwaway migration check were found and removed; no container and no volume remains |
| R45-h | **What ζ's green run demonstrates, in the database rather than in a claim** | Two runs of one workflow with identical input took different paths, because the planner fetched a step's output through the read API; a `static` workflow ran beside the `http` one and its decisions are rows too (`§ 12.1`'s no-exemption rule); an unreachable planner, a slow one and a 500 produced `transport_error`, `timeout` and `transport_error` on their own rows while sharing one dead-letter reason; a declared `fail` was recorded as a **successful** call answering fail, costing no budget; and a run that died at its first decision point was replayed, asked again, and finished — with the failed round's calls still carrying `replay_round = 0` |
| R45-i | **Still outstanding** | The owner's own hand-run of ζ (`CLAUDE.md § 4` step 5) — which is what a green suite is explicitly *not* evidence for; `§ 18.5`; R41-i (the slow-planner subtest, his to cut); the commit of rounds 44–45 and everything since `e9a5e9e`; and R40-q's carry-overs |

---

## Round 46 — every demo script run before the owner is asked to run one (2026-09-10)

| # | Topic | Ruling / outcome |
|---|---|---|
| R46-a | **Owner asked to be taught how to hand-verify every milestone, ζ in particular** | The guide was written for him in conversation: what each stage is testing, what to type, what he must see, and why each observation is evidence rather than decoration. It is not written into the repository — `CLAUDE.md § 1`'s document set has no place for an operator guide, and `notes/` is explanatory material rather than instructions to follow |
| R46-b | **Before teaching it, the five demo scripts were run — because they had been edited today and never executed since** | `CLAUDE.md § 5.4` says tests run "always before the owner is asked to look at a milestone by hand", and handing him a script that had only been syntax-checked would have made his time the place a defect surfaced. This is the same obligation applied to the artefact he actually runs |
| R46-c | **It found one: γ's demo still asserted `dead_letter_queue.attempt_count`** | γ's **suite** was corrected in round 45; its **demo** was not, and nothing had run it since. The two are separate files serving the same milestone, and fixing one is not fixing the other. The demo now reads the number from the table that owns it — a count of the attempts carrying that entry's `replay_round` — and prints that count for the operator's eye beside the entry |
| R46-d | **A trap that produced a false green, recorded because it fired three times in one session** | `cmd \| tail -12` reports **tail's** exit status, not the command's. A sweep written that way declared `exit=0` for three demos whose result blocks it had not even captured. The re-run captures the script's own status into a variable before anything is piped. Together with R45-f — a heredoc silently consumed before `python3` saw it — the lesson is one lesson: **a shell reporting success is evidence about the shell, not about the work** |
| R46-e | **Results: all five demos pass** | α and β: no failed assertion, exit 0. δ: **51 assertions, 0 failures**. ζ: **94 assertions, 0 failures**. γ after the fix: exit 0, no failures. No container and no volume left behind by any of them |
| R46-f | **What this does and does not discharge** | It discharges *"the script he is about to run has been run"*. It does **not** discharge `CLAUDE.md § 4` **step 5**, which is his own eyes on the database — `SPEC.md § 17.4` says why in one sentence: "a green suite the owner has never seen behind is not evidence that a milestone landed", and a green demo is no different |
| R46-g | **Still outstanding** | ζ's hand-run by the owner; `§ 18.5`; R41-i (the slow-planner subtest, his to cut); pushing the five unpushed commits; and R40-q's carry-overs |

---

## Round 47 — ζ hand-verified by the owner; the work is pushed and θ opens (2026-09-10)

| # | Topic | Ruling / outcome |
|---|---|---|
| R47-a | **The owner ran ζ by hand and reported it finished** | This is the one thing a green suite and a green demo script are explicitly *not* evidence for (`CLAUDE.md § 4` step 5, `SPEC.md § 17.4`, R46-f). With it, milestone ζ has landed: the custom HTTP planner, the switch between planner types, and the planner fetching a step's output through the read API have all been seen in the database by the owner's own eyes. R45-i's and R46-g's first outstanding item is discharged. **What this entry records is his report, not this agent's observation** — nothing here restates database contents that were not put in front of it |
| R47-b | **Ten scripts appeared modified, and nothing had changed** | Read through the Windows UNC path, `git status` reported `demos/*/demo.sh` and `test/*/run.sh` as mode changes, `100755` → `100644`. The same repository read from inside WSL is clean. The exec bits are intact; the Windows-side filesystem layer simply cannot see them. **It came within one command of being committed as a real regression** — a commit that would have stripped the executable bit from every script the owner runs by hand. `CLAUDE.md § 8`'s rule is written about builds, containers and migrations; this is the same rule reaching one step further, to *reading state* as well as changing it. Git is now run through WSL like everything else |
| R47-c | **A zero-byte `demos/zeta/kind` left behind by the hand-run** | Untracked, empty, and created during the manual session — a shell artefact, not an input any environment references. Removed rather than committed; nothing was in it to lose |
| R47-d | **Six commits pushed** | `10dccee` through `a2ed7e9`, plus this round's entry: the `fetch_base_url` transcription, ζ's environment, the planner-call amendment, its implementation, the per-mode output rule, and γ's demo fix. R46-g said five; `10dccee` had been unpushed longer than the round that counted them |
| R47-e | **θ opens, and `CLAUDE.md § 4` step 1 was done before anything else: its rules are already ratified** | `SPEC.md § 18` orders it sixth — *sync raw body: an unmodifiable HTTP endpoint works as a worker*. `§ 9.6` settles both halves of it without a new ruling. Success and failure: **any 2xx succeeds, any non-2xx fails, and the truncated body is stored as error text** — there is no envelope, so the transport status *is* the verdict. What is stored: the **entire** response body verbatim, as R45-b made explicit. `§ 9.6`'s transport-versus-business paragraph closes the remainder — both burn one attempt alike — so θ introduces no state-machine rule that α and γ have not already exercised |
| R47-f | **What θ therefore needs, and where the line falls** | Not new behaviour: an environment (`demos/theta/`) whose worker is a plain HTTP endpoint that knows nothing about Piton, a suite derived from `§ 9.6` and `§ 9.7`, and a demo script. That script's **section in `SPEC.md § 18` is the owner's to direct, not this agent's to write** (`CLAUDE.md § 2` rule 1) — and it is worth him seeing that `§ 18` currently stops at `18.3`: **δ's and ζ's sections were never written**, which is what the outstanding `§ 18.5` has been standing for. Two milestones he has hand-verified have no demo-script section in the one document that is an authority on behaviour |
| R47-g | **Still outstanding** | `§ 18.4` and `§ 18.5` — δ's and ζ's demo-script sections — and in due course θ's; R41-i (the slow-planner subtest, his to cut); and R40-q's carry-overs |

---

## Round 48 — θ's first question: what a raw worker is allowed to answer (2026-09-10)

| # | Topic | Ruling / outcome |
|---|---|---|
| R48-a | **`CLAUDE.md § 4` step 1 found one gap, and it is a real one** | θ's rules are otherwise ratified: `§ 9.5` fixes the raw request body as `params` verbatim and nothing else, `§ 9.6` fixes success and failure (any 2xx succeeds; any non-2xx fails and the truncated body becomes error text), `§ 9.7` makes sync + raw legal, and `§ 9.8` rule 4 rejects `input_from` beside it. The gap is what happens when a raw worker answers **2xx with a body that is not JSON**. `§ 9.6` says the entire body verbatim is the output; `§ 6.3` and `§ 6.4` call an output *JSON bytes* and the schema makes both columns `JSONB`. A `text/plain`, XML or CSV reply satisfies one sentence and cannot be stored under the other |
| R48-b | **Owner ruled: a raw worker MUST answer JSON, and a 2xx that is not JSON is `invalid_response`** | It burns one attempt like any other failure. The ruling needs **no change to `§ 5.3`**, which already defines `invalid_response` as *"a reply arrived, but could not be parsed as the mode requires"* — the mode now requires JSON, and the existing words cover it. That the vocabulary already fitted is part of why this reading was the one recommended |
| R48-c | **The two alternatives, and why each was worse** | Wrapping a non-JSON body in a JSON string is Piton shaping the worker's result, which `§ 6.3` forbids in as many words, and it cannot tell a worker that returned the string `"hello"` from one that returned the bare text `hello`. Widening the columns to bytes would mean a migration against two columns under five hand-verified milestones, a rewrite of `§ 6.3` and `§ 6.4`'s wording, and a fresh question `§ 9.5` does not answer: what `inputs` means when it hands a non-JSON output to the next worker |
| R48-d | **Owner ruled the raw request's header: `content-type: application/json`** | `§ 9.5` draws the body and says nothing about the header. `§ 9.4` already requires `params` to be an object defaulting to `{}`, so a raw request is a JSON request and now says so. A per-StepSpec header field was offered and declined as `CLAUDE.md § 3`'s optional-field case, which belongs in `BACKLOG.md`. Settling it now rather than later is that same section's admission test: it is a wire-format semantic |
| R48-e | **One existing sentence is now false as written** | `§ 9.5`'s why-clause promises that *"any unmodifiable HTTP endpoint — an existing internal API, a third-party API — is then a valid worker"*. After R48-b that is *any such endpoint **that answers JSON***. The clause is not deleted: the reason `raw` exists is unchanged, and it is the limit that is new. Leaving the sentence as it stands would leave `SPEC.md` promising something θ deliberately does not deliver |
| R48-f | **Nothing is written into `SPEC.md` yet, and no test exists yet** | `CLAUDE.md § 2` rule 1 forbids an unsolicited SPEC edit however obviously correct, and `§ 5.1` forbids deriving a test from a spoken ruling before it reaches SPEC. The exact amendment — two places in `§ 9.5`, one new paragraph in `§ 9.6` — was drafted and put to the owner for his direction to transcribe. θ's suite and demo begin after that, in `CLAUDE.md § 4`'s order |

---

## Round 49 — θ's environment and suite, written before the code (2026-09-11)

| # | Topic | Ruling / outcome |
|---|---|---|
| R49-a | **Owner approved the round 48 amendment and directed the tests** | The three places drafted at R48-f were transcribed unchanged and committed on their own as `c57484e`, before a line of θ existed — `CLAUDE.md § 5.1` forbids deriving a test from a spoken ruling, so the rule had to be in `SPEC.md` first. No section was renumbered |
| R49-b | **`demos/theta/` has four services rather than α's three, and the fourth is the milestone** | `rawapi` is written the way a third-party API is written: it does not know what a run, a step or an attempt is, never looks for `status` or `output`, and has no idea anything is retrying it. Piton's own envelope worker stays beside it, because a raw-only environment would show that raw dispatch works and nothing about what it is *for* — `§ 9.5`'s `inputs` has no exemption for a step that ran in raw mode, and that claim needs one worker of each inside a single run |
| R49-c | **Why the endpoint echoes what it received** | `§ 9.5`'s raw rules — the body is `params` and nothing else, sent as `application/json` — are claims about a wire that leaves no trace. By answering with what it saw, the wire becomes a value in `steps.output` that SQL can assert and the owner can read at a terminal (`§ 17.1`). Echoing a request back is something an ordinary API does; it is not Piton knowledge |
| R49-d | **Two groups, and the negative assertions carry most of the weight** | `rawpath` asserts that the raw body was `params` **and that none of Piton's six envelope fields appeared in it** — an implementation that sent the envelope *and* the params would satisfy the positive half alone. It also asserts the sharpest rule θ has: a raw step whose endpoint answers 2xx with `{"status":"failure",…}` is a **success**, stored whole, because the dispatch style decides whether Piton's protocol words mean anything. `rawfailure` asserts the two failures only raw can produce, and that they stay **distinguishable** — `§ 5.3`'s reason for the column |
| R49-e | **No `validation` group and no `ownership` group, deliberately** | `test/alpha/validation` already asserts all six rules of `§ 9.8`, rule 4 (`input_from` beside `raw`) included, and θ asserts no global coordination state, which is the only thing `§ 5.5.3` reserves a separate group for. `CLAUDE.md § 5` makes scope subtractive; duplicating those cases would mean two places to fix when one rule changes |
| R49-f | **The suite was run before the code exists, and its failure has the right signature** | `rawpath`: every assertion fails, and the run reaches DLQ. `rawfailure`: the not-JSON run was labelled `transport_error` rather than `invalid_response`, and `TestTheTwoFailuresAreDistinguishable` reports that both runs were collapsed into one reason. That is exactly what `internal/dispatch` does today — it answers every raw step with an explicit *"milestone theta and is not implemented in this build"* transport failure, so nothing was ever sent to the endpoint. `CLAUDE.md § 4` step 3 and R20-a: this is the expected state, not a defect |
| R49-g | **Running it found one defect of this agent's own, which is why it was run** | `attempts` and `steps` both carry a `status` column, and one joined assertion left it unqualified: psql answered *"column reference status is ambiguous"* rather than true or false. A suite written and never executed would have carried that until the implementation landed, where it would have looked like a bug in the code |
| R49-h | **A false alarm this agent raised against `run.sh`, and the real lesson in it** | A probe appeared to show `run.sh` exiting 0 while `go test` printed FAIL — a false green under every milestone. It was not true. `$?` was being substituted by the Windows-side shell **before the text reached WSL**, so every exit code this agent measured that way read 0, including `false`. Re-checked with `&&`/`||`, which need no `$?`, `run.sh` propagates correctly and R45-g's five green suites stand. This is R45-f and R46-d a third time, from a third direction: **the measurement was reporting on the measuring apparatus** |
| R49-i | **A stale column found in two demo scripts, in the path nobody runs** | `dead_letter_queue.attempt_count` was deleted by the round 43 amendment. α's `demo.sh` still selected it inside `diagnose()`, which runs **only when something has already failed** — a trap set for the next failing hand-run. δ's `demo.sh` selected it in its **main flow**, where `show` ends in `|| true`, so it printed a psql error instead of the operator's evidence for `§ 6.5` and the script still exited 0. R46-e's *"δ: 51 assertions, 0 failures"* was true and that evidence block was broken at the same time. Both now count the attempts carrying the entry's `replay_round`, which is R45-e and R46-c's ruled substitution applied to the two copies that had not had it. A repository-wide sweep confirms no reader of the deleted column remains |
| R49-j | **Still outstanding** | **`demos/theta/demo.sh` — `CLAUDE.md § 4` step 2, not yet written**, so θ is not ready to be put in front of the owner; the implementation itself; `§ 18.4`, `§ 18.5` and in due course θ's demo-script section; R41-i; and R40-q's carry-overs |

---

## Round 50 — θ implemented; suite and demo green, and a regression this agent had already shipped (2026-09-12)

| # | Topic | Ruling / outcome |
|---|---|---|
| R50-a | **Owner directed the order and it was followed: tests, then the demo script, then the code** | `demos/theta/demo.sh` was written as `CLAUDE.md § 4` step 2 asks — the commands the operator types and the database state he must see — and only then was `internal/dispatch` touched |
| R50-b | **The implementation is one file and one idea** | `internal/dispatch` builds the body per `§ 9.5` (raw sends `spec.Params` verbatim, envelope sends the marshalled envelope), sends both with `content-type: application/json`, shares one non-2xx path, and on a 2xx in raw mode checks `json.Valid` before storing the body whole. `internal/engine` needed **nothing**: `assembleInputs` already returned an empty map for a non-envelope style, citing `§ 9.4`, and the 4 KB truncation already ran in the storage driver. The α author had left θ's seams in place |
| R50-c | **The mode gate got simpler rather than larger** | It had three branches — sync+envelope, a raw rejection, a default ε rejection. It is now one condition: anything that is not `sync` is ε. `§ 9.8` rule 3 already refuses async+raw at submission, so the combination cannot reach this function, and the gate no longer pretends to guard against it |
| R50-d | **Results, in the order they were produced** | Suite: `rawpath` 11 tests, `rawfailure` 6, **all 17 green**. Demo: **53 assertions, 0 failures**, three legs — DONE, DLQ, DLQ — with `transport_error` and `invalid_response` on their own runs. Not one assertion was edited to make this happen; `CLAUDE.md § 5.2` forbids it and nothing tempted it |
| R50-e | **The suite was red first, on purpose, and that is on the record** | Before the code it reported every `rawpath` assertion failing and the not-JSON run labelled `transport_error` — because `internal/dispatch` answered every raw step with an explicit not-implemented failure and nothing reached the endpoint. `test/theta/README.md` now says this in the past tense rather than as a warning, because the sequence is the evidence that `§ 4` step 3 was obeyed |
| R50-f | **A regression this agent had already committed and pushed** | `98671ab` changed `demos/alpha/demo.sh` and `demos/delta/demo.sh` from `100755` to `100644`. The Edit tool, writing through the Windows path, dropped the executable bit, and `git add -A` committed the mode change along with the content fix. **This is R47-b's consequence arriving by the opposite route**: that round caught Windows-side git *reading* a false mode; this one is a Windows-side editor *writing* a real one |
| R50-g | **It was caught by running the artefact, not by reading it** | θ's own `demo.sh` lost its bit the same way and answered `./demo.sh: Permission denied` on its first run after the edits. Had the demo not been run, three scripts the owner runs by hand would have shipped unexecutable, and `CLAUDE.md § 4` step 5 would have failed in his terminal rather than in this agent's. All twelve `demo.sh` and `run.sh` files were swept; the filesystem and the index now agree at `100755` |
| R50-h | **The rule this produces, stated so it is not learned a third time** | Any script edited through a Windows-side tool has its mode checked from inside WSL before it is staged. `CLAUDE.md § 8` says the Windows side is an editor only; what neither it nor R47-b said is that an editor can change a file's *mode* as well as its bytes |
| R50-i | **One thing for the owner, not decided here** | `SPEC.md § 19.3`'s table — *"deliberately unbuilt but designed in"* — still carries the row `raw dispatch | § 9.5 | Milestone θ`. With θ implemented that row is false. `CLAUDE.md § 2` rule 1 forbids an unsolicited edit, so it is raised and left alone |
| R50-j | **R49-h's defect bit a third time, and the working rule is now written rather than remembered** | A six-suite regression sweep reported all five milestones `FAILED` in under a second. The loop variable had been substituted away before the text reached WSL — exactly as `$?` was at R49-h — so what actually ran was `./test//run.sh`, five times. **Any shell variable in a command sent through the Windows-side wrapper may be expanded by the wrong shell.** The rule that follows: a loop or a `$?` check goes into a **file**, written once and executed inside WSL, never into a command string. Verified the same way as everything else — by reading the file back and confirming the variables survived, before running it |
| R50-k | **Still outstanding** | The owner's own hand-run of θ (`§ 4` step 5), which is the only thing a green suite and a green demo are explicitly *not* evidence for; `§ 19.3`'s stale row; `§ 18.4`, `§ 18.5` and θ's demo-script section; R41-i; and R40-q's carry-overs |

---

## Round 51 — the one section that goes stale by working correctly (2026-09-12)

| # | Topic | Ruling / outcome |
|---|---|---|
| R51-a | **Owner asked what "a section of SPEC is false" meant, and the answer was worse than the flag had suggested** | `§ 19.3` is titled *"Deliberately unbuilt but designed in"* and its third column is *"When it is built"*. It carried the row `raw dispatch | § 9.5 | Milestone θ` — a claim that raw dispatch was not built, contradicted by the commit that had just built it. It also carried `HTTP planner | § 6.1 | Milestone ζ`, **false since round 44 and unnoticed for four rounds**, through a hand-verification and five suite sweeps |
| R51-b | **`git log -S` settled why: the table had never been updated, not once** | It was written in `55f4a8a` on 2026-09-01, before any milestone was implemented, and no commit has touched those rows since. This is not a maintenance lapse that happened twice; it is a section nobody has ever maintained |
| R51-c | **The structural reason, which is the part worth keeping** | `§ 19.3` is the only place in `SPEC.md` that describes **what exists in the code** rather than **how the system must behave**. Everywhere else `CLAUDE.md § 1` holds: *if the code and the document disagree, the code is wrong*. Here the rule inverts — writing the code is precisely what makes the row false — so it is the one section that **goes stale by someone doing their job correctly**. Nothing in `CLAUDE.md § 4`'s six steps made anyone responsible for it, so it could only ever rot |
| R51-d | **Owner ruled: remove the built rows, and add the step** | `§ 19.3` now lists four things and states the discipline in the section itself — *a row leaves this table when the thing is built* — naming the two that have already left and why the rule needs saying. `CLAUDE.md § 4` gains **step 7**: sweep `SPEC.md` for statements the landing has made false and propose the correction, with the ζ miss recorded underneath as the reason the step exists |
| R51-e | **Step 7 was then executed for the first time, and found a second one** | `§ 18.2` — γ's demo-script section, ratified and hand-verified — says *"the only way to reach L5 **today** is an unbuilt planner reporting that it is unbuilt"*. That was true when γ was designed and is not true now: ζ's HTTP planner exists and can genuinely fail. The paragraph's conclusion still holds and the next sentence already points at ζ, so the whole defect is one present-tense word. **Proposed, not edited** (`CLAUDE.md § 2` rule 1): `today` becomes `when γ was designed`, nothing else changes |
| R51-f | **What the sweep did not find** | The seven *"must be null until milestone η"* clauses in `§ 9.4` and `§ 9.8` read like status claims and are not: they are behaviour rules binding the validator today, true whatever exists in the code, and they stay |
| R51-g | **Still outstanding** | The owner's own hand-run of θ (`§ 4` step 5); the `§ 18.2` wording above; `§ 18.4`, `§ 18.5` and θ's demo-script section; R41-i; R40-q's carry-overs; and two commands the permission classifier refused this agent — the remote rename to `StateFlow` and the push of `a28c03f` |

---

## Round 52 — the read API, and a fourth document (2026-09-12)

| # | Topic | Ruling / outcome |
|---|---|---|
| R52-a | **Owner asked for a front end, and the first job was to check what another agent had told him** | A separate session had audited the API and reported four missing endpoints. Three of its claims did not survive checking. **It said attempt detail was invisible over HTTP** — `GET /runs/{run_id}/steps` has always returned the full attempt array, which is the single most dramatic thing the owner wanted to show. It **missed two endpoints** (`GET /workflows` and `GET /workflows/{id}`, both `§ 10.1`). And of its four, only **two** actually block a front end: `GET /steps/{id}` is redundant with the catalogue and `POST /runs/{id}/cancel` is milestone ι's behaviour, not a read endpoint |
| R52-b | **`SPEC.md § 2.2` says a user interface is out of scope, and that is not a veto** | It reads *"the interface is the database and the HTTP read API, seen from a terminal"* — out of scope for **Piton**, not forbidden. `BACKLOG.md` B7 settles it: `GET /ui` was *"not cut, only deprioritised — terminal-first is the requirement, not terminal-only. May still be useful for demos"*. A front end is a **consumer** of the read API, which `§ 10`'s opening line already anticipates: "the read endpoints are shared by the operator and the planner; they are one surface, not two" |
| R52-c | **Nothing needed to be added to `SPEC.md`'s endpoint list, which was the shape-changing fact** | Both endpoints the front end needs are already ratified in `§ 10.2`, and that section closes with *"these endpoints are required early, not late"*. Implementing them cashes in existing design — the same distinction that made R43-m decline a planner-calls endpoint, because `§ 10.2` does not list one |
| R52-d | **Two things were genuinely awkward to add later, and those are what was amended** | `§ 10.2` said only *"list runs, filterable by status"*. It now fixes `GET /runs`'s whole contract — the repeatable `status`, `limit` 1–200 defaulting to 50, an opaque `cursor`, and the ordering **newest first by `created_at` with `run_id` breaking ties** — because a consumer that discovers pagination in its second year rewrites its client (`CLAUDE.md § 3`). And one sentence settles the dead-letter endpoint's empty case: **a run with no entries is an empty list with a 200, not a 404**, since `§ 10.5` reserves 404 for an entity that does not exist |
| R52-e | **A fourth document, and a new row in `CLAUDE.md § 1`** | `SPEC.md` has never fixed a response field name — deliberately, and the α harness records why: *"SPEC.md 10.2 does not fix the JSON field name that carries a run's status … so polling the API here would mean inventing a wire contract no ruling covers."* For six milestones every consumer was a planner or an operator at a terminal, and `§ 17.1` made database truth the interface. A browser has neither, so the field names became a contract, and a contract needs somewhere to live. `API.md` is **authority on the wire only**: it may never state a rule about behaviour, it cites the SPEC section that rules each value's meaning, and where the two disagree SPEC wins and `API.md` is wrong. Every field in it names the database column it carries |
| R52-f | **The owner's own idea replaced this agent's design, and was better** | This agent had proposed session-keyed chaos control so that two viewers could not collide. He proposed instead that the demo worker and planner each carry a `/pause` and `/resume` endpoint. It is simpler, needs no state machine, and **needs no change to Piton at all** — the orchestrator keeps POSTing to the same `worker_url`; what changes is what answers. It also produces something no earlier milestone could: every one of them declares its failure in `params` before the run starts, so the run is doomed from birth. Pausing interrupts a run that was **already healthy** |
| R52-g | **Paused means silent, not refusing — and that is a `§ 5.3` distinction, not a detail** | A paused service accepts the connection and writes nothing, so the attempt expires against `deadline_at` and is classified `timeout`. A service that refused the connection would be `transport_error`. `§ 5.3` decides between them **by the clock, not by the shape of the error**, and its why-clause is explicit that collapsing the two makes the operator "misdiagnose a dead worker as a slow one" |
| R52-h | **Five-second timeouts, and why that is configuration rather than a compromise** | `§ 11.1` ranges `step_timeout_seconds` and `planner_timeout_seconds` at `>= 1` and defaults them to 300 and 30. At the defaults, pausing a worker means watching nothing for five minutes. `demos/console/workflow.json` sets 5 — legal, no code, and `§ 13.3` guarantees that while the run's owner is live failure is declared **precisely** at the deadline rather than somewhere after it |
| R52-i | **The cursor is a keyset, and the reason is in the amendment the owner approved** | `§ 10.2` requires that a cursor "resumes **exactly** after the run it names", which an offset cannot promise: the list is newest-first, so every new run inserts at the front and pushes every later offset down — a walk of a table still being written to silently repeats rows. The demo's whole point is viewers starting runs, so this is the common case rather than the exotic one. The `run_id` tiebreak is the other half: two runs created in the same instant have no defined order under `created_at` alone, and a page boundary inside that tie either skips one or repeats one |
| R52-j | **`step_id` is a pointer so that a planner-side entry serialises as `null`** | `§ 6.5` makes it NULL for a planner-side entry, and the reason is not tidiness: when a planner never answers, **no step was ever created** — the run died at a decision point, before it had any work to do. A Go `string` has no absent state, so the field would have reached a client as `""`, which cannot be told apart from "a step exists and its id came through blank". Different situations, different client behaviour, and the wire has to carry the difference |
| R52-k | **The suite's fixture is four runs, and two of them are a pair** | A collection endpoint cannot be exercised by one run. The worker-side and planner-side DLQ runs exist together because **both end `DLQ` and `runs.status` cannot tell them apart** — the dead-letter `reason` is the only thing that can, which is exactly what `§ 12.3` separates the two sides for and what this endpoint was designed to carry |
| R52-l | **Results** | The console suite: **14 of 14 green**, written before the handlers existed. Full regression across α γ β δ ζ θ and console: **all seven green**, which mattered because this change touched the storage interface and the shared attempt summary |
| R52-m | **One field that existed in the database and not on the wire** | `attempts.replay_round` (`§ 6.4`). `§ 14` resets `steps.attempt_count` on replay and `attempt_no` stays contiguous across rounds, so nothing else in the response distinguished a round-0 attempt from a round-1 one. Added to the attempt summary — a response field, not a SPEC matter, since SPEC fixes no field names |
| R52-n | **Still outstanding** | The owner's hand-run of θ **and** of the console demo (`§ 4` step 5); `§ 18.2`'s stale *"today"* (R51-e); `§ 18.4`, `§ 18.5` and θ's demo-script section; R41-i; R40-q's carry-overs; and the remote rename to `StateFlow`, which the permission classifier refuses this agent |

---

## Round 53 — the pause switch is keyed by run, and the deployment rules are written down (2026-09-12)

| # | Topic | Ruling / outcome |
|---|---|---|
| R53-a | **Owner ruled the pause switches per-run, and the logic ships with the demo bundle** | A single global switch works for one operator at a terminal and fails the moment two people watch the same demo: one presses pause and the other's run dies for a reason they did not cause — **and it dies convincingly**, with real `timeout` attempts and a real dead-letter entry, so nothing on screen says it was somebody else's doing. `§ 9.5`'s envelope and `§ 9.2`'s planner request both carry `run_id`, so the two services can go silent for exactly one run. **Piton is untouched**: it keeps POSTing to the same `worker_url` and what changes is what answers |
| R53-b | **The first design made a race unavoidable, and the suite caught it on its first run** | `/resume` with no run also cleared every per-run pause, so narrowing "everything paused" to "one run paused" took two requests — and the hold loop re-reads the state every 200 ms, so the victim escaped through the gap and reached `DONE` instead of `DLQ`. **The fix was the semantics, not a sleep in the test**: each switch now does exactly one thing, `/resume` lifts only the global flag, and `/reset` is what clears everything. The planner's copy had the identical defect and was green — its timeout path is slower and simply did not land in the window — so both were fixed on the evidence of one failure |
| R53-c | **`test/console/isolation` is its own group** | It creates runs while others are in flight and leaves some dead on purpose, which `readapi` — a group that counts runs — could not tolerate. `CLAUDE.md § 5.5.3`'s reasoning, applied to a fixture rather than to coordination state. Nothing in `SPEC.md` mentions a pause switch; what the assertions cite is what it **produces**: `§ 5.3`'s `timeout`, `§ 12.2`'s march to DLQ, and `§ 6.5`'s null `step_id` on the planner side |
| R53-d | **`docker-compose.prod.yml` is an overlay, not an edit** | `docker-compose.yml` is the file `test/console` runs against, and `CLAUDE.md § 5.5.4` requires the suite and the owner's hand-run demo to exercise an identical definition. The overlay withdraws every published port and puts Caddy in front — one public port, a shared-secret header, three internal destinations by path |
| R53-e | **Owner pushed back on the `worker_url` allowlist, and he was right** | Told that the proxy should validate `worker_url` against an allowlist, he asked whether that would not make the demo useless — nobody could run their own worker. It would have. `§ 9.5` exists **so that any unmodifiable HTTP endpoint is a valid worker**, and an allowlist naming only this compose file's services keeps the host safe by throwing the demonstration away |
| R53-f | **The rule is a property of the address, not a list of hosts** | Resolve the hostname; refuse loopback, the three private ranges, link-local (`169.254.0.0/16`, where cloud metadata sits) and their IPv6 equivalents; **allow everything else**. A visitor can still point a step at their own public endpoint, which is the whole demonstration, and the dangerous targets are gone rather than enumerated |
| R53-g | **Why an unchecked `worker_url` is worse than a blind request** | The worker's response becomes the step's stored output (`§ 6.3`), and the same caller reads it back through `GET /steps/{step_id}/output`. It does not merely reach an internal address — **it returns what it found there** |
| R53-h | **Owner asked why the orchestrator does not police this itself. It must not** | Two reasons, both ratified. `§ 9.5` makes POSTing to an arbitrary URL the product rather than a defect; an orchestrator that refused arbitrary URLs would stop being what this system is. And `§ 2.2` puts authorisation out of scope entirely — "may *this caller* aim at *that* address" is an authorisation question, and **Piton has no notion of a caller**. The danger is a property of the deployment, so the check belongs where the deployment knows who is asking |
| R53-i | **The two gates, and why neither can do the other's job** | Caddy holds the secret and cannot read request bodies; the application proxy reads bodies and is not on the host. Coverage is complete only because the key is the sole way in: every `POST /workflows` must pass through the proxy that holds it |
| R53-j | **The residual risk is written down rather than implied** | DNS rebinding defeats check-then-connect: the name resolves public when the proxy checks and private when the orchestrator connects. Closing it means resolving once and connecting to the pinned IP, which Piton does not do. Recorded in `demos/console/README.md` beside the condition that makes it acceptable — a host that runs nothing else |
| R53-k | **Where the rules live, and why no fourth document** | `demos/console/README.md` for the deployment account, `API.md`'s security section for the client author who implements the check. `CLAUDE.md § 1` gained one row this session already; a second new voice for deployment notes is not worth it when a demo directory's README is already the place that explains that demo |
| R53-l | **Results** | The console suite **16 of 16 green** across both groups |
| R53-m | **Still outstanding** | The owner's hand-run of θ **and** of the console demo (`§ 4` step 5); the endpoint-by-endpoint check through Caddy that he asked for; `§ 18.2`'s stale *"today"* (R51-e); `§ 18.4`, `§ 18.5` and θ's demo-script section; R41-i; R40-q's carry-overs; and the remote rename to `StateFlow` |
