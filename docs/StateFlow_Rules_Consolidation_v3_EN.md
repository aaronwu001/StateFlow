...
```
History is seq-ascending and carries each step's full output. **Wire-casing rule: every status string on the wire is UPPERCASE ("DONE"), identical to the stored values.** **The MVP sends the full history — registered as the system's weakest link (§19):** LLM context bloat and DB row bloat are real risks; the committed slimming paths (summary-plus-fetch, pass-by-reference) are scheduled fast-follows. Users must currently design planners on the premise "you will receive the full history."
